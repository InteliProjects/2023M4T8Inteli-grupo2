...
app.post("/addEntrega", (req, res) => {
  ...
  console.log(req.body, "add entrega");
...
app.post("/addAtivo", (req, res) => {
  console.log(req.body, "add ativo");
...
app.post("/hist", (req, res) => {
  ...
  // Use a parameterized query for safety and correctness
  const query = `SELECT issueDate,deliveryDate,destination,status,reason FROM hist WHERE id = ?`;
  db.all(query, [id], (err, rows) => {
    if (err) {
      console.error(err.message);
      res.status(500).json({ message: "Internal server error" });
...
`;
  db.run(query, [status, id, id], function (err) {
    if (err) {
      console.error(err.message);
